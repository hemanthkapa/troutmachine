...
export default function Machine() {
    ...
    const handleLucky = () => {
        const prompt = WHIMSICAL_PROMPTS[Math.floor(Math.random() * WHIMSICAL_PROMPTS.length)];
        ...
        <div className={`machine-container ${status}`}>
            ...
            {status !== 'idle' && (
                <header className="machine-header">
                    <img src="/trout-logo-large.png" alt="The Trout Gallery" className="header-logo" />
                    <h1 className="results-title">{title}</h1>
                    <button className="search-again-link" onClick={reset}>search again</button>
                </header>
            )}
            ...
            {status === 'idle' && (
                <div className="state-idle fade-in">
                    <div className="idle-content">
                        <img src="/trout-logo-large.png" alt="The Trout Gallery" className="hero-logo" />
                        <h1 className="hero-title">mysterious trout machine</h1>
                        <p className="hero-subtitle">enter a phrase to reveal hidden connections.</p>

                        <form onSubmit={handleSearch}>
                            <input
                                type="text"
                                placeholder="two birds and one stone"
                                value={query}
                        ...
                        <div className="actions">
                            <button className="lucky-btn" onClick={handleLucky}>i'm feeling lucky</button>
                        </div>

                        <div className="bottom-action">
                            <button className="search-link" onClick={handleSearch} disabled={!query}>search the collection →</button>
                        </div>
                    </div>
            ...
            {status === 'results' && !selectedArtwork && (
                <div className="state-results fade-in">
                    {artworks.filter(art => art.image_url && art.title && art.artist).length > 0 ? (
                        <div className="gallery-track">
                            {artworks
                                .filter(art => art.image_url && art.title && art.artist)
                                .map((art) => (
                                    <div key={art.id} className="artwork-card">
                                        <div className="image-wrapper" onClick={() => handleSelectArtwork(art)}>
                ...
                <div className="detail-view fade-in">
                    ...
                    <div className="detail-content">
                        <div className="detail-image"
                            ...
                            style={{ position: 'relative', overflow: 'hidden' }}> {/* Added relative positioning */}

                            <img src={selectedArtwork.image_url} alt={selectedArtwork.title} style={{ cursor: 'crosshair' }} />
                            ...
                            {zoomProps.show && (
                                <div
                                    ...
                                    style={{
                                        ...
                                        backgroundPosition: `${(zoomProps.x / zoomProps.width) * 100}% ${(zoomProps.y / zoomProps.height) * 100}%`,
                                        backgroundSize: `${zoomProps.width * 2}px ${zoomProps.height * 2}px`, // 2x Zoom relative to formatted image size
                                    }}
                                />
                            )}
                        </div>
                        <div className="detail-metadata">
                            ...
                            <div className="meta-grid">
                                ...
                                <div className="meta-item">
                                    ...
                                    <span>{selectedArtwork.dimensions || 'Dimensions unavailable'}</span>
                                </div>
                                <div className="meta-item">
                            ...
                            <a href={selectedArtwork.url || '#'} target="_blank" rel="noopener noreferrer" className="met-link">View on Trout Gallery Website →</a>
                        </div>
                    </div>
                </div>
            )}
